  ...
  for (let i = 0; i < articles.length - 1; i++) {
    ...
    if (currentTime && nextTime) {
      ...
      if (currentTime < nextTime) {
        ...
        const contextEnd = Math.min(articles.length, i + 4);
        const contextArticles = articles.slice(contextStart, contextEnd).map((article, idx) => ({
          position: contextStart + idx + 1,
          rank: article.rank,
          time: article.rawTime,
          title: article.title.substring(0, 40) + (article.title.length > 40 ? '...' : ''),
          isErrorArticle: (contextStart + idx === i) || (contextStart + idx === i + 1)
        }));
        ...
        issues.push({
          ...
          errorType: 'chronological_order_violation'
        });
      } else {
        validCount++;
      }
    } else {
      // Handle cases where timestamp parsing failed
      if (!currentTime || !nextTime) {
        const contextStart = Math.max(0, i - 1);
        const contextEnd = Math.min(articles.length, i + 3);
        const contextArticles = articles.slice(contextStart, contextEnd).map((article, idx) => ({
          position: contextStart + idx + 1,
          rank: article.rank,
          time: article.rawTime,
        ...
        issues.push({
          ...
          context: contextArticles,
          errorType: 'timestamp_parsing_failed',
          parseError: !currentTime ? 'current' : 'next'
        });
      }
    }
  }
